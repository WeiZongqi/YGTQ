...
regressor = LassoCV(normalize=False, n_jobs=-1, alphas=np.arange(0.009,0.019,0.0002), cv=ShuffleSplit(n_splits=50,test_size=0.05))

regressor, scaler = regressor_train(regressor, train_data, train_score, auxiliary_data=None, test_data=test_data,
                                    normalize='categorical', discrete_col=new_col_name, max_z_score=2.5, discrete_max_z_score=4.5,
                                    discrete_weight=20)
